...
def parse_year(date):
    ...
    print('now_second:',now_second)
    # 设置某个时间点
    time = datetime.date(now_year - date, now_month, now_day)  # 年，月，日
    print('time:',time)
    # 将时间转换成指定格式
...
# parse_date('8月22日  23:50')


def process_date(value):
    if '分钟前' in value:
        res = num_pattern.search(value).group()
        date = (datetime.datetime.now() - timedelta(days=int(res))).strftime('%Y-%m-%d %H:%M:%S')
    elif '小时前' in value:
        res = num_pattern.search(value).group()
        date = (datetime.datetime.now() - timedelta(days=int(res))).strftime('%Y-%m-%d %H:%M:%S')
    elif '天前' in value:
