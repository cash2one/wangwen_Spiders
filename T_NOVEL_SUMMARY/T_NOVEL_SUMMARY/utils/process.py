...
def parse_time(date):
    ...
    if re.match('今天.*', date):
        dates = re.match('今天(.*)', date).group(1).strip()
        print('dates:',dates)
        date = time.strftime('%Y-%m-%d', time.localtime(time.time())) + ' ' + dates
    if re.match('昨日.*', date):
        dates = re.match('昨日(.*)', date).group(1).strip()
        print('dates:',dates)
        date = time.strftime('%Y-%m-%d', time.localtime(time.time() - 24 * 60 * 60)) + ' ' + dates
    if "昨天" in date:
    ...
    if re.match('\d+年前',date):
        ...
        date = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time() - float(year) * 12 * 30.41665 * 24 * 60 * 60))
        print('date:',date)
    if re.match('\d{2}-\d{2}', date):
        date = time.strftime('%Y-', time.localtime()) + date + ' 00:00:00'
    # if re.match('\d{2}月\d{2}日\s\d{2}\:\d{2}', date):
