...
class HeiyanSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        author = ''.join(response.xpath('//div[@class="author-zone column-2"]/div[@class="right"]/a[@class="name"]//text()').extract()).strip()
        item["author"] = author
        print('author:', author)
        novel_type = ';'.join(response.xpath('//p[@class="infos"]/span[@class="cate"]/a/text()').extract()).strip()
        ...
        item["Signed"] = Signed
        # print('Signed:', Signed)
        novel_desc = ''.join(response.xpath('//div[@class="summary min-summary-height"]/pre[@class="note"]/text()').extract()).strip()
        ...
        P_image = ''.join(response.xpath('//div[@class="pic"]/a/img/@src//div[@class="pic"]/a/img/@src').extract()).strip()
